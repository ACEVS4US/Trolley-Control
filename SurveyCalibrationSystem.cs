...
namespace Trolley_Control
{
    ...
    public partial class Tunnel_Control_Form : Form
    {
            ...
            if (this.InvokeRequired == false)
            {
                switch (procNum)
                {
                    ...
                    case ProcNameSerialCom.POLL:
                        ...
                        {
                            if (!barometer.ErrorReported)
                            {
                                //There has been a send receive error....update the GUI
                                MessageBox.Show(msg.ToString());
                                barometer.ErrorReported = true; //avoid unneccessary reporting
                            }
                        }
                        break;
                ...
                switch (procNum)
                {
                    ...
                    case ProcNameHumidity.SEND_RECEIVE:
                        if (msg.Equals("No Error"))
                        {
                            ...
                            double result1 = TH_logger1.getH;
                            double result2 = TH_logger2.getH;
                        ...
                        if (file_valid)
                        {
                            ...
                            switch (name)
                            {
                                ...
                                case "NUMBER SAMPLES":
                                    ...
                                    Measurement_list[current_measurement_index].setAveraging = true;
                                    break;
                                case "DIRECTION":
        ...
        private void EDMRadioButton_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if(!EDMRadioButton.Checked&& !TotalStationRadioButton.Checked && !AuxLaserRadioButton.Checked)
            {
                EDMRadioButton.Checked = true;
            }
            else
            {
                //The EDM button wasn't checked....but it is now.  The Aux laser is currently also checked - uncheck it
                if (EDMRadioButton.Checked && !TotalStationRadioButton.Checked && AuxLaserRadioButton.Checked)
                {
                    Measurement.SetDeviceType(Device.EDM);
                    AuxLaserRadioButton.Checked = false;
                }
                //The EDM button wasn't checked....but it is now.  The Total Station is currently also checked - uncheck it
                else if (EDMRadioButton.Checked && TotalStationRadioButton.Checked && !AuxLaserRadioButton.Checked)
                {
                    Measurement.SetDeviceType(Device.EDM);
                    TotalStationRadioButton.Checked = false;
                }
        ...
        private void TotalStationRadioButton_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (!EDMRadioButton.Checked && !TotalStationRadioButton.Checked && !AuxLaserRadioButton.Checked)
            {
                TotalStationRadioButton.Checked = true;
            }
            else
            {
                //The Total Station button wasn't checked....but it is now.  The Aux laser is currently also checked - uncheck it
                if (!EDMRadioButton.Checked && TotalStationRadioButton.Checked && AuxLaserRadioButton.Checked)
                {
                    Measurement.SetDeviceType(Device.TOTAL_STATION);
                    AuxLaserRadioButton.Checked = false;
                }
                //The Total Station button wasn't checked....but it is now.  The EDM is currently also checked - uncheck it
                else if (EDMRadioButton.Checked && TotalStationRadioButton.Checked && !AuxLaserRadioButton.Checked)
                {
                    Measurement.SetDeviceType(Device.TOTAL_STATION);
                    EDMRadioButton.Checked = false;
                }
            }
        }
        ...
        private void AuxLaserRadioButton_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (!EDMRadioButton.Checked && !TotalStationRadioButton.Checked && !AuxLaserRadioButton.Checked)
            {
                AuxLaserRadioButton.Checked = true;
            }
            else
            {
                //The Aux Laser button wasn't checked....but it is now.  The Total Station is currently also checked - uncheck it
                if (!EDMRadioButton.Checked && TotalStationRadioButton.Checked && AuxLaserRadioButton.Checked)
                {
                    Measurement.SetDeviceType(Device.TOTAL_STATION);
                    AuxLaserRadioButton.Checked = false;
                }
                //The Aux Laser button wasn't checked....but it is now.  The EDM is currently also checked - uncheck it
                else if (EDMRadioButton.Checked && !TotalStationRadioButton.Checked && AuxLaserRadioButton.Checked)
                {
                    Measurement.SetDeviceType(Device.TOTAL_STATION);
                    EDMRadioButton.Checked = false;
        ...
        private void redrawLaserEnviroTextbox()
        {
            ...
            try
            {
                ...
                LaserParameters.AppendText("WaveLength: " + Measurement.LaserWavelength.ToString() + " nm\n");
                LaserParameters.AppendText("DUT WaveLength: " + Measurement.DUTWavelength.ToString() + " nm\n");
                LaserParameters.AppendText("Average Tunnel Air Temp: " + avg_temp.ToString() + " °C\n");
                LaserParameters.AppendText("Average Laser Beam Temp: " + avg_temp2.ToString() + " °C\n");
                LaserParameters.AppendText("Average EDM Beam Temp: " + avg_temp3.ToString() + " °C\n");
                LaserParameters.AppendText("Air Pres: " + Measurement.Pressure.ToString() + " hPa\n");
                LaserParameters.AppendText("Average %RH: " + Measurement.AverageHumidity.ToString() + "%\n");
                LaserParameters.AppendText("Agilent RI Corr: " + HP5519_Laser.AirCompensation.ToString() + "\n");
                LaserParameters.AppendText("Phase Refractive Index (Laser): " + Measurement.CalculatePhaseRefractiveIndex(Measurement.LaserWavelength).ToString() + "\n");
                LaserParameters.AppendText("Group Refractive Index: " + Measurement.CalculateGroupRefractiveIndex(850).ToString() + "\n");
                LaserParameters.AppendText("CO2 Level: " + Measurement.CO2.ToString() + "\n\n");
                ...
                if (a != null)
                {
                    ...
                    LaserParameters.AppendText("Laser PRTs Used: " + laser_prts.ToString() + "\n");
                    ...
                    switch (DUT.Beamfolds)
                    {
                        case 0:
                            ...
                            LaserParameters.AppendText("EDM PRTs Used: " + one_beam_prts.ToString() + "\n\n");
                            break;
                        case 1:
                            break;
                        case 2:
                ...
                for (int i = 0; i < 30; i++)
                {
                    LaserParameters.AppendText("Temperature PRT "+ (i+1).ToString()+": " + Measurement.getTemperature(i).ToString() + "\n");
                }
            ...
            if (okay)
            {
                    ...
                    while (true)
                    {
                        ...
                        else if (line_read.Contains("BRIDGE NAME:"))
                        {
                            ...
                            c_agilent.Correction = getBridgeCorrection(line_read);
                            bridge = c_agilent;
                        continue;
                        }
